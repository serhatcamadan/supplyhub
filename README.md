...
| Backend API | NestJS 12 (Railway) — [`supplyhub-api`](https://github.com/serhatcamadan/supplyhub-api) |
...
- NestJS JWT (access token 15dk + refresh token 7gün, httpOnly cookie)
...
├── app/
│   ├── [locale]/
│   │   ├── (auth)/          → Login, Signup (4 adımlı + OTP)
│   │   ├── seller/          → Satıcı portalı sayfaları
│   │   └── buyer/           → Alıcı portalı sayfaları
│   └── api/
│       ├── auth/            → NestJS proxy (login, logout, signup, send-verification)
...
├── lib/
│   ├── api/                 → NestJS client (auth, products)
│   ├── auth/                → JWT decode (server + client)
...
- [x] NestJS backend (Auth, Products, Orders, QuoteRequests modülleri)
...
- [ ] FAZ 3.6 — RBAC (RolesGuard, company-based erişim kontrolü)
- [ ] FAZ 3.7 — Kalan veri sorguları NestJS'e taşıma (mock → gerçek)
